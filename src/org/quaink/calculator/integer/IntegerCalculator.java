...
public class IntegerCalculator extends JFrame implements MouseListener {
	...
	public static void main(String[] args) {
		/* 窗体 */
		integerCalculator.setTitle("整数计算器 Designed by QuainK"); // 窗体标题
		integerCalculator.setSize(600, 550); // 窗体大小
		integerCalculator.setResizable(false); // 窗体可调大小禁用
	...
	public void inputString(String s) {
		/* 字符串叠加，比如输入 "1+2*3"，就是从 1，1+，1+2，1+2* 到 1+2*3 */
	...
	public void printData() {
		...
		for (i = 0; i <= operationStackIndex; i++) {
			System.out.printf("操作符[%d] = %s\t", i, operationStack[i]);
		}
		for (i = 0; i <= numberStackIndex; i++) {
			System.out.printf("操作数[%d] = %f\t", i, numberStack[i]);
		}
		System.out.print("\n");
	}
}
